...
engine_overheat_protection::EngineOverheatProtection::Config CreateDefaultConfig() {
    ...
    config.torque_lookup_table_size = sizeof(oil_temp_torque_limit_table) / sizeof(oil_temp_torque_limit_table);

    return config;
}

void TestIdleWhenEngineStopped() {
    ...
    auto creat_default_config = CreateDefaultConfig();
    CycleInput InputArray;

    signals::FloatSignal oil_temp(InputArray.oil_temp, InputArray.oil_temp_validity);
    signals::BoolSignal is_engine_running(InputArray.is_engine_running, InputArray.is_engine_running_validity);

    engine_overheat_protection::EngineOverheatProtection epo_test (
