...
const char* OnOffStr(signals::OnOffStatus s) {
    return (s == signals::OnOffStatus::ON) ? "ON" : "OFF";
}
...
int main() {
    ...
    signals::BoolSignal is_diag_normal(true, signals::ValidityStatus::VALID);
    ...
    config.IncrementTimerConfig.threshold_timer = 3;
